...
import { AlertCircle, InfoIcon, Loader2, X } from "lucide-react";
...
export default function PresentationWizard() {
  ...
  const onComplete = async (data: any, presentationId?: string) => {
    // The presentationId has already been created and saved by the PresentationWizard
    // when transitioning from Step 2 to Step 3 (via CreatePresentation API)

    // Redirect to presentation page with presentationId in dynamic route
    if (presentationId) {
      router.push(`/presentation/${presentationId}`);
    } else {
      // Fallback - redirect to home if no presentationId
      router.push("/");
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#1F1F1F]">
      <Header />
      <div className="mx-auto max-w-207.25 px-4 py-8.75">
